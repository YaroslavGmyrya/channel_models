...
pdf_rice = x * np.exp(-(t**2 + A**2)/(2))*i0(x*A) # theoretical pdf of channel
...
plt.title("RX rectangle signal")
plt.xlabel("time, sec")
plt.ylabel("A, V")

plt.subplot(5, 1, 3)
plt.hist(np.abs(rx_signal), bins=40, density=True)
plt.plot(x, pdf_rice, label='Theoretical Rayleigh PDF')
plt.xlabel("X")
plt.ylabel("P(X)")
